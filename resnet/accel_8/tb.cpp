						...
						for(int h = 0; h < KERNEL[0]; h++) {
							for(int coo = 0; coo < 16; coo++, idx++) {
								...
								for(int cii = 0; cii < CIN[0]; cii++) {
									...
									temp_32 = weights_float[0][widx] * 256;
									temp8 = temp_32 << SHIFT_W[0];
									params[idx].range(BIT_W_8*cii+BIT_W_8-1, BIT_W_8*cii) = temp8.range(BIT_W_8-1, 0);
								}
							}
						}
					}
					...
					for(int h = 0; h < KERNEL[l]; h++) {
						for(int coo = 0; coo < 16; coo++, idx++) {
							...
							for(int cii = 0; cii < 32; cii++) {
								...
								temp_32 = weights_float[l][widx];
								temp8 = temp_32 << SHIFT_W[l];
								params[idx].range(BIT_W_8*cii+BIT_W_8-1, BIT_W_8*cii) = temp8.range(BIT_W_8-1, 0);
							}
						}
					}
				...
				for(int w = 0; w < KERNEL[l]; w++) {
					for(int h = 0; h < KERNEL[l]; h++) {
						for(int coo = 0; coo < 16; coo++, idx++) {
							...
							for(int cii = 0; cii < 32; cii++) {
								widx = (co+coo)*(CIN[l]*KERNEL[l]*KERNEL[l])+(ci+cii)*(KERNEL[l]*KERNEL[l])+w*KERNEL[l]+h;
								temp_32 = weights_float[l][widx] / 49;
								temp8 = temp_32 << SHIFT_W[l];
								params[idx].range(BIT_W_8*cii+BIT_W_8-1, BIT_W_8*cii) = temp8.range(BIT_W_8-1, 0);
							}
						}
		...
		for(int co = 0; co < COUT[21]; co += 16) {
			for(int coo = 0; coo < 16; coo++, idx++) {
				...
				for(int cii = 0; cii < 32; cii++) {
					...
					temp_32 = weights_float[21][widx];
					temp8 = temp_32 << SHIFT_W[21];
					params[idx].range(BIT_W_8*cii+BIT_W_8-1, BIT_W_8*cii) = temp8.range(BIT_W_8-1, 0);
				}
			}
		}
...
int main() {
	...
	//if(read_images("images1000.dat")) return -1;
	printf("Reading parameters\n");
	if(read_params("resnet18_float.dat")) return -1;
	...
	//if(write_reordered_images("images_reordered_new.dat")) return -1;
	printf("Writing reordered parameters\n");
	if(write_reordered_params("resnet18_8b_2_dynamic_new.dat")) return -1;
	printf("Reading reordered images\n");
	if(read_reordered_images("images_reordered.dat")) return -1;
	//printf("Reading reordered parameters\n");
	//if(read_reordered_params("resnet18_8b_2_dynamic_new.dat")) return -1;
	...
	for(int idx = 2; idx < NUM_TESTS; idx++) {
		accel_8(images_fix[idx], buffer, params, probs);
