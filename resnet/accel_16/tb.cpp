...
int read_labels(char const* filename) {
	...
	fil = fopen(filename, "rb");
	if(!fil) return -1;
	ret = fread((void*)labels, sizeof(int), NUM_TESTS, fil);
	if(!ret) return -1;
	ret = fclose(fil);
	if(ret) return -1;
...
int main() {
	...
	for(int idx = 2; idx < NUM_TESTS; idx++) {
		accel_16(images_fix[idx], buffer, params, probs);
