...
int write_reordered_params(char const* filename, int num_params) {
    int ret;
    FILE *fil = 0;
	fil = fopen(filename, "wb");
	if(!fil) return -1;
...
int main() {
	...
	if(write_reordered_params("mobilenet_reordered.dat", num_params)) return -1;
	printf("Reading reordered images\n");
	if(read_reordered_images("images_reordered.dat")) return -1;
	printf("Reading reordered parameters\n");
	if(read_reordered_params("mobilenet_16b.dat")) return -1;*/
	...
	for(int idx = 0; idx < NUM_TESTS; idx++) {
		...
		for(int l = 0; l < NUM_LAYERS; l++) {
			layer_info(info, l);
			accel_16(images_fix[idx], buffer, params, probs, info);
		}
		accel_16(images_fix[idx], buffer, params, probs, info);
